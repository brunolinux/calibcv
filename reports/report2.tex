...
\\
\\
Also, in this stage we recognized the order of features detected. We started calculating the center of pattern given by the average coordinates of all features, then we computed features which are located in the corners of the pattern given by the farthest features from the center point. Finally, we detected the intermediate points, using informations of distances between lines that pass through two corners and rest of points; we used this information to give a number to every point in pattern.
\\
\\
%% TODO: Gerson
\begin{algorithm}
